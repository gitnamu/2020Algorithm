...
public class FindClosetPair {
    ...
    public PairOfPoints solveByConquer(PointSet pointSetForSolve) {
        if (pointSetForSolve == null || pointSetForSolve.size() <= 1) {
            return null;
        }
    ...
    private SeparatedPair separateReferenceList(ReferenceList referenceList, int separationLine) {
        ...
        int separationX = FindClosetPair.this.pointSet().elementAt(separationLine).x;
        ...
        for (int index = 0; index < sizeOfReferenceList; index++) {
            ...
            Point pointReferencedByIndex_i = FindClosetPair.this.pointSet().elementAt(pointReference);
